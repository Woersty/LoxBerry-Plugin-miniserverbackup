...
foreach ($ms as $msno => $miniserver ) 
{
	...
	if ( count($filetree["name"]) > 0 )
	{
 		...
 		foreach( $filetree["name"] as $k=>$file_to_save)
		{
			...
			{
				debug(__line__,str_ireplace("<file>",$bkpfolder.$file_to_save,str_ireplace("<dwl_size>",$filesize($workdir_tmp."/".$bkpfolder.$file_to_save),str_ireplace("<ms_size>",$filetree["size"][array_search($file_to_save,$filetree["name"],true)],$L["ERRORS.ERR_0013_DIFFERENT_FILESIZE"]))),4);
				sleep(1); 
